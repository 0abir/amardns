...
impl SafeBrowsingClient {
    ...
    pub async fn check_domain(&self, domain: &str) -> Option<String> {
        ...
        if clean == "testsafebrowsing.appspot.com"
            || clean.ends_with(".testsafebrowsing.appspot.com")
        {
            threat_entries.push(
                serde_json::json!({ "url": "http://testsafebrowsing.appspot.com/s/malware.html" }),
            );
            threat_entries.push(
                serde_json::json!({ "url": "https://testsafebrowsing.appspot.com/s/malware.html" }),
            );
            threat_entries.push(
                serde_json::json!({ "url": "http://testsafebrowsing.appspot.com/s/phishing.html" }),
            );
        }
                    ...
                    if let Ok(body) = resp.json::<Value>().await {
                        if let Some(matches) = body.get("matches").and_then(|m| m.as_array()) {
                            if !matches.is_empty() {
                                ...
                                self.threat_cache
                                    .insert(clean.to_string(), threat_type.clone())
                                    .await;
...
mod tests {
    ...
    async fn test_safe_browsing_cache() {
        ...
        client
            .threat_cache
            .insert("malicious.site".to_string(), "MALWARE".to_string())
            .await;
        assert_eq!(
            client.check_domain("malicious.site").await,

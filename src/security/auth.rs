...
/// Constant-time string comparison that prevents timing attacks and length leakage.
/// Hashes both strings with SHA-256 and compares the fixed 32-byte digests in constant time.
pub fn constant_time_eq_str(a: &str, b: &str) -> bool {
    let hash_a = ring::digest::digest(&ring::digest::SHA256, a.as_bytes());
    let hash_b = ring::digest::digest(&ring::digest::SHA256, b.as_bytes());
    let mut diff = 0u8;
    for (&x, &y) in hash_a.as_ref().iter().zip(hash_b.as_ref().iter()) {
        diff |= x ^ y;
    }
    diff == 0
}
...
mod tests {
    ...
    fn test_check_auth_roles() {
        ...
        headers.insert(
            ...
            "Bearer secret_master_key".parse().unwrap(),
        );

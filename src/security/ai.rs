...
impl AIBrain {
    ...
    pub fn extract_features(&self, domain: &str) -> ([f32; 8], f32) {
        ...
        let f6 =
            if domain.contains("paypal") || domain.contains("google") || domain.contains("apple") {
                0.8
            } else {
                0.0
            };
...
mod tests {
    ...
    fn test_ai_brain_learning_and_decision() {
        ...
        assert!(
            score < 0.6,
            "Clean domain should have low threat score, got {}",
            score
        );

...
#[inline]
fn stripe_for_identity(key: &IdentityKey) -> usize {
...
#[inline]
fn stripe_for_ip(ip: IpAddr) -> usize {
    use std::hash::{Hash, Hasher};
    let mut hasher = std::collections::hash_map::DefaultHasher::new();
    ip.hash(&mut hasher);
    (hasher.finish() as usize) % STRIPE_COUNT
...
///
/// Thread-safe via 32-way striped [`parking_lot::Mutex`] partitions to eliminate lock contention.
pub struct RateLimiter {
    config: RateLimitConfig,

    /// 32-way striped Per-identity (IP + device_id) buckets.
    identity_stripes: [Mutex<IdentityMap>; STRIPE_COUNT],

    /// 32-way striped Per-IP ceiling buckets (shared by all identities from that IP).
    ip_ceiling_stripes: [Mutex<IpCeilingMap>; STRIPE_COUNT],
...
impl RateLimiter {
    ...
    pub fn new(
        ...
        Self {
            ...
            identity_stripes: std::array::from_fn(|_| {
                Mutex::new(IdentityMap {
                    buckets: HashMap::new(),
                })
            }),
            ip_ceiling_stripes: std::array::from_fn(|_| {
                Mutex::new(IpCeilingMap {
                    buckets: HashMap::new(),
                })
            }),
    ...
    pub fn check_with_identity(&self, ip: IpAddr, device_id: Option<&str>) -> bool {
        ...
        // ── 2. Per-identity bucket (partitioned across 32 striped locks) ─────
        let identity_stripe_idx = stripe_for_identity(&key);
        let identity_ok = {
            let mut map = self.identity_stripes[identity_stripe_idx].lock();
            let bucket = map
                .buckets
                .entry(key)
        ...
        // ── 3. Per-IP ceiling bucket (partitioned across 32 striped locks) ───
        let ip_canonical = ip.to_canonical();
        let ceiling_ok = if identity_ok {
            let ip_stripe_idx = stripe_for_ip(ip_canonical);
            let mut map = self.ip_ceiling_stripes[ip_stripe_idx].lock();
            let bucket = map.buckets.entry(ip_canonical).or_insert_with(|| {
    ...
    /// Prunes token buckets across all 32 partitions that have not been accessed in the last 60 seconds.
    pub fn prune_idle(&self) {
        let now = Instant::now();
        for stripe in &self.identity_stripes {
            let mut map = stripe.lock();
            map.buckets
                .retain(|_, b| now.duration_since(b.last_seen()).as_secs() < 60);
        }
        for stripe in &self.ip_ceiling_stripes {
            let mut map = stripe.lock();
            map.buckets
                .retain(|_, b| now.duration_since(b.last_seen()).as_secs() < 60);
        }
    ...
    /// Clears all rate-limiter buckets across all 32 partitions to immediately release memory during pressure.
    pub fn clear(&self) {
        for stripe in &self.identity_stripes {
            stripe.lock().buckets.clear();
        }
        for stripe in &self.ip_ceiling_stripes {
            stripe.lock().buckets.clear();
        }
...
mod tests {
    ...
    fn private_and_loopback_ips_always_pass() {
        ...
        for &ip in cases {
            for _ in 0..200 {
                assert!(limiter.check(ip), "Expected exempt IP {ip} to always pass");
            }
        }
    ...
    #[test]
    fn test_ipv4_mapped_ipv6_exemption_and_identity_parity() {
        let limiter = tight_limiter(1.0, 1.0);
        // IPv4-mapped private IPs must be recognized as exempt
        let mapped_private: IpAddr = "::ffff:172.16.65.82".parse().unwrap();
        let mapped_loopback: IpAddr = "::ffff:127.0.0.1".parse().unwrap();
        assert!(limiter.check(mapped_private));
        assert!(limiter.check(mapped_loopback));
        assert_eq!(limiter.get_blocked_count(), 0);

        // Identity key equality between IPv4 and IPv4-mapped IPv6
        let v4: IpAddr = "103.187.94.29".parse().unwrap();
        let mapped_v4: IpAddr = "::ffff:103.187.94.29".parse().unwrap();
        let k1 = IdentityKey::new(v4, Some("dev-1"));
        let k2 = IdentityKey::new(mapped_v4, Some("dev-1"));
        assert_eq!(k1, k2);
    }

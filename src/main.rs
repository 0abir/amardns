...
async fn async_main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tokio::spawn(async move {
        ...
        match boot_state.upstreams.sync_and_rank().await {
            Ok(count) => {
                ...
                boot_state.log_config_decision(
                    "upstream_pool_ranked",
                    "QUALIFIED_9",
                    0.05,
                    0.0,
                    1,
                    serde_json::json!({
                        "qualifiedPool": { "from": "INITIALIZING", "to": format!("{} active nodes", count) },
                        "routingEngine": { "from": "-", "to": "ranked_lowest_latency" }
                    }),
                );
                info!(
    ...
    tokio::spawn(async move {
        ...
        let mut last_lb_mode = "FAST".to_string();
        ...
        loop {
            ...
            // Autonomous Traffic & Profile Adaptation
            let rps = probe_state.metrics.get_rps();
            let stress = probe_state.metrics.calc_stress(rps);
            let cur_mode = if rps > 100.0 || stress > 0.8 {
                "RELIABLE"
            } else if stress < 0.3 {
                "FAST"
            } else {
            ...
            if cur_mode != last_lb_mode {
                let dev_count = probe_state.metrics.active_device_count();
                probe_state.log_config_decision(
                    "traffic_adaptation",
                    cur_mode,
                    stress,
                    rps,
                    dev_count as u32,
                    serde_json::json!({
                        "lbMode": { "from": last_lb_mode, "to": cur_mode },
                        "strategy": {
                            "from": if last_lb_mode == "FAST" { "single_primary" } else { "hedged_race" },
                            "to": if cur_mode == "RELIABLE" { "turbo_all_race" } else if cur_mode == "BALANCED" { "hedged_race" } else { "single_primary" }
                        }
                    }),
                );
                last_lb_mode = cur_mode.to_string();
            }
        }
    });
    ...
    // - Normal Zone (< Yellow Zone): Cache runs dynamically at peak capacity (150k entries)
    //   maximizing cache hits and eliminating upstream latency.
    // - Yellow Zone (>= 70% of TOTAL_MEM_CAP): Proactively evicts expired/idle cache entries and
    //   prunes stale rate-limiter buckets and returns freed pages via malloc_trim.
    // - Red Zone (>= 87.5% of TOTAL_MEM_CAP): Emergency cache shedding to guarantee the TOTAL_MEM_CAP hard ceiling
    //   is never breached, resetting RSS down to ~BASE_MEM immediately.
    let mem_gov_state = state.clone();
    let total_cap = config.total_mem_cap;
    let red_zone = (total_cap * 0.875 * 10.0).round() / 10.0;
    let yellow_zone = (total_cap * 0.70 * 10.0).round() / 10.0;
    tokio::spawn(async move {
        ...
        loop {
            ...
            if rss_mb >= red_zone {
                ...
                tracing::warn!(
                    "[mem-governor] RED ZONE REACHED: RSS = {:.1} MB (>= {:.1} MB). Executing emergency memory shedding!",
                    rss_mb,
                    red_zone
                );
            ...
            } else if rss_mb >= yellow_zone {
                // YELLOW ZONE: Proactive maintenance
                if last_warning_time.elapsed().as_secs() >= 30 {
                    tracing::info!(
                        "[mem-governor] Yellow zone (RSS = {:.1} MB >= {:.1} MB). Running maintenance eviction sweep.",
                        rss_mb,
                        yellow_zone
                    );
    ...
    if config.acme_enabled {
        ...
        let acme_cfg = server::acme::AcmeConfig {
            ...
            desec_domains: config.desec_domains.clone(),
            duckdns_domains: config.duckdns_domains.clone(),
            dynu_domains: config.dynu_domains.clone(),

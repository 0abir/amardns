...
pub struct QueryLog {
    ...
    pub dnssec_alg: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub dnssec_key_tag: Option<u16>,
    #[serde(default)]
    pub ad_flag: bool,
}
...
impl AppState {
    ...
    pub fn new(config: Config) -> Self {
        ...
        let initial_decisions = vec![ConfigDecision {
            t: now,
            decision: "system_boot_tuning".to_string(),
            profile: "FAST".to_string(),
            stress: 0.05,
            rps: 0.0,
            user_est: 1,
            changed: serde_json::json!({
                "lbMode": { "from": "COLD_BOOT", "to": "FAST" },
                "hedgeDelay": { "from": "-", "to": "8ms / 20ms" },
                "circuitBreaker": { "from": "-", "to": "45% threshold" },
                "cacheWireCap": { "from": "-", "to": "150,000 entries" }
            }),
        }];

        let initial_actions = vec![ActionLog {
            t: now,
    ...
    /// Asynchronously synchronizes the latest IANA Root Zone Trust Anchors from https://data.iana.org/root-anchors/root-anchors.xml
    pub async fn sync_dnssec_root_anchors(&self) {
        let client = reqwest::Client::builder()
            .timeout(std::time::Duration::from_secs(10))
            .build()
            .unwrap_or_default();
        let cache_path = if std::path::Path::new("/data").exists() {
            "/data/root-anchors.xml"
        } else {
            "root-anchors.xml"
        };
        let _ = self
    ...
    pub fn log_config_decision(
        ...
        let now = SystemTime::now()
            ...
            .unwrap_or_default()
            .as_millis() as u64;
        let mut guard = self.config_decisions.write();
        guard.push(ConfigDecision {
            t: now,
    ...
    pub fn log_query(
        ...
        self.log_query_dnssec(
            domain, qtype, client, proto, status, rcode, lat_ms, reason, upstream, "INSECURE",
            None, None, false,
        );
    }

    #[allow(clippy::too_many_arguments)]
    pub fn log_query_dnssec(
        &self,
        domain: &str,
        qtype: u16,
        client: &str,
        ...
        upstream: &str,
        dnssec_status: &str,
        dnssec_alg: Option<&str>,
        dnssec_key_tag: Option<u16>,
        ad_flag: bool,
    ) {
        let now = SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
        ...
        if self.log_broadcaster.receiver_count() > 0 {
            let is_threat = status != "ALLOW"
                || (!reason.is_empty() && reason != "none")
                || dnssec_status == "BOGUS";
    ...
    pub fn check_domain(&self, domain: &str) -> (bool, bool, &'static str) {
        ...
        if let Some(_reason) = self.schedule_store.is_blocked_now(&clean) {
            ...
            self.fast_neg_filter
                .insert(clean.clone(), ("schedule_block", false));
        ...
        if self.is_threat_bloom(&clean) {
            self.fast_neg_filter
                .insert(clean.clone(), ("threat_feed_abir", true));
            self.metrics.threat_blocks.fetch_add(1, Ordering::Relaxed);
        ...
        if crate::security::heuristics::is_lookalike_threat(&clean) {
            ...
            self.log_anomaly(
                "lookalike_threat",
                &format!("Brand impersonation phishing: {}", clean),
            );
        ...
        if crate::security::heuristics::is_dga_threat(&clean) {
            ...
            self.record_detected_block(&clean, "DGA");
        ...
        if score > 0.90 {
            ...
            self.log_anomaly(
                "neural_zero_day",
                &format!(
                    "Autonomous Zero-Day Shield ({:.0}% score): {}",
                    score * 100.0,
                    clean
                ),
            );
    ...
    pub fn is_custom_blocked(&self, domain: &str) -> Option<&'static str> {
        self.is_exact_blocked(domain)
            .or_else(|| self.is_wildcard_custom_blocked(domain))
    }
    ...
    pub fn record_detected_block(&self, domain: &str, reason: &str) {
        ...
        let (tag, source, auto) =
            if reason.contains("dga") || reason.contains("lookalike") || reason.contains("ai") {
                ("AI", "ai", true)
    ...
    /// Revokes an individual HMAC token by registering its token/signature string.
    #[allow(dead_code)]
    ...
    pub fn is_token_revoked(&self, token: &str, issued_ts: u64) -> bool {
        if issued_ts < self.token_revocation_epoch.load(Ordering::Acquire) {
            return true;
        }
        let guard = self.revoked_tokens.read();
...
pub fn domain_matches_pattern(pattern: &str, domain: &str) -> bool {
    ...
    if !clean_pat.contains('*') && clean_dom.ends_with(&format!(".{}", clean_pat)) {
        return true;
    }
...
impl AppState {
    ...
    pub async fn sync_threat_feeds(&self) -> Result<(usize, usize), String> {
        ...
        let block_url_cdn = format!(
            "https://cdn.jsdelivr.net/gh/abir614/-@latest/blocklist.txt?_t={}",
            now_sec
        );
        let block_url_fastly = format!(
            "https://fastly.jsdelivr.net/gh/abir614/-@latest/blocklist.txt?_t={}",
            now_sec
        );
        let block_url_raw = format!(
            "https://raw.githubusercontent.com/abir614/-/main/blocklist.txt?_t={}",
            now_sec
        );

        let white_url_cdn = format!(
            "https://cdn.jsdelivr.net/gh/abir614/-@latest/whitelist.txt?_t={}",
            now_sec
        );
        let white_url_fastly = format!(
            "https://fastly.jsdelivr.net/gh/abir614/-@latest/whitelist.txt?_t={}",
            now_sec
        );
        let white_url_raw = format!(
            "https://raw.githubusercontent.com/abir614/-/main/whitelist.txt?_t={}",
            now_sec
        );

        let tb_url_cdn = format!(
            "https://cdn.jsdelivr.net/gh/abir614/-@latest/total_blocked.txt?_t={}",
            now_sec
        );
        let tb_url_raw = format!(
            "https://raw.githubusercontent.com/abir614/-/main/total_blocked.txt?_t={}",
            now_sec
        );

        let tw_url_cdn = format!(
            "https://cdn.jsdelivr.net/gh/abir614/-@latest/total_whitelisted.txt?_t={}",
            now_sec
        );
        let tw_url_raw = format!(
            "https://raw.githubusercontent.com/abir614/-/main/total_whitelisted.txt?_t={}",
            now_sec
        );
        ...
        let (b_opt, w_opt, tb_opt, tw_opt) = tokio::join!(
            fetch_feed_text(
                &client,
                vec![block_url_cdn, block_url_fastly, block_url_raw]
            ),
            fetch_feed_text(
                &client,
                vec![white_url_cdn, white_url_fastly, white_url_raw]
            ),
        ...
        if block_count > 0 {
            *self.threat_bloom.write() = new_threat_bloom;
            tracing::info!(
                "[threat-feed] Double-buffered hot-swap complete: {} domains loaded (0 downtime).",
                block_count
            );
            if expected_block > 0 {
                self.expected_threat_total
                    .store(expected_block, Ordering::Relaxed);
            } else {
                self.expected_threat_total
        ...
        if white_count > 0 {
            ...
            *self.whitelist_wildcards.write() = new_wildcards;
            if expected_white > 0 {
                self.expected_whitelist_total
                    .store(expected_white, Ordering::Relaxed);
            } else {
                self.expected_whitelist_total
                    .store(white_count, Ordering::Relaxed);
            }
        }

        if block_count > 0 && white_count > 0 {
            self.feed_overlap_count
                .store(crossmatched_overlap, Ordering::Relaxed);
        }
    ...
    pub fn trigger_background_feed_sync(self: &std::sync::Arc<Self>) {
        ...
        {
            tracing::info!(
                "[threat_feed] Background feed sync is already active; pooling in progress"
            );
            return;
        }
        ...
        tokio::spawn(async move {
            ...
            loop {
                attempt += 1;
                tracing::info!(
                    "[threat_feed] Background feed sync attempt #{} starting...",
                    attempt
                );
...
mod tests {
    ...
    fn test_normalize_blocklist_line() {
        assert_eq!(
            normalize_blocklist_line("*.000webhost.com"),
            Some("000webhost.com".to_string())
        );
        assert_eq!(
            normalize_blocklist_line("||doubleclick.net^"),
            Some("doubleclick.net".to_string())
        );
        assert_eq!(
            normalize_blocklist_line("0.0.0.0 bad-domain.ru"),
            Some("bad-domain.ru".to_string())
        );
    ...
    async fn test_threat_feed_subdomain_blocking() {
        ...
        state
            .custom_whitelist
            .write()
            .insert("login.000webhost.com".to_string());
        let (blocked_wl, _, reason_wl) = state.check_domain("login.000webhost.com");
        assert!(
            !blocked_wl,
            "Whitelisted domain must bypass threat feed bloom filter!"
        );
    ...
    async fn test_cdn_total_parsing_and_crossmatching() {
        ...
        let parsed_b: usize = sample_blocked_raw
            .chars()
            .filter(|c| c.is_ascii_digit())
            .collect::<String>()
            .parse()
            .unwrap();
        let parsed_w: usize = sample_white_raw
            .chars()
            .filter(|c| c.is_ascii_digit())
            .collect::<String>()
            .parse()
        ...
        state
            ...
            .store(parsed_b, Ordering::Relaxed);
        state
            .expected_whitelist_total
            .store(parsed_w, Ordering::Relaxed);
    ...
    async fn test_exact_block_punches_hole_through_wildcard_whitelist() {
        ...
        assert!(
            blocked,
            "Exact block must punch through wildcard whitelist!"
        );
        assert_eq!(reason, "custom_block");
        assert!(
            !state.is_exempt("analytics.apple.com"),
            "Exact blocked domain must not be exempt"
        );
        assert!(
            state.is_domain_blocked("analytics.apple.com"),
            "Exact blocked domain must be domain blocked"
        );
        ...
        assert!(
            state.is_exempt("music.apple.com"),
            "Other subdomains under wildcard whitelist must be exempt"
        );
        ...
        assert!(
            blocked_feed,
            "Threat bloom exact subdomain must punch through wildcard whitelist!"
        );
        ...
        state
            .custom_whitelist
            .write()
            .insert("analytics.apple.com".to_string());
    ...
    async fn test_sync_threat_feeds_crossmatched_flag_logic() {
        ...
        assert!(
            !is_crossmatched,
            "0 rules must NEVER report crossmatched=true"
        );
    ...
    #[tokio::test]
    async fn test_uptimate_production_test() {
        // === UPTIMATE PRODUCTION TEST ===
        // Comprehensive end-to-end production audit verifying every critical subsystem:
        ...
        assert!(role.is_view_or_admin());
        assert!(!role.is_admin());
        assert_eq!(role, crate::security::auth::AuthRole::View);

        // 4. PulseDB WAL Storage Engine
        state.wal.append_block("malicious-actor.xyz");
        state.wal.append_whitelist("legitimate-service.org");
        tokio::time::sleep(std::time::Duration::from_millis(150)).await;
        assert!(state.wal.total_records() >= 2);
        assert_eq!(state.wal.dropped_entries.load(Ordering::Relaxed), 0);

        // 5. High-Performance Bloom Filters (2M Capacity with Near-Zero FP)
        assert_eq!(state.threat_bloom.read().capacity_bits(), 134_217_728);
        assert_eq!(state.threat_bloom.read().memory_bytes(), 16_777_216);
        assert_eq!(state.whitelist_bloom.read().capacity_bits(), 1_048_576);
        state.threat_bloom.write().insert("confirmed-c2-botnet.cc");
        assert!(state.threat_bloom.read().contains("confirmed-c2-botnet.cc"));
        assert!(!state.threat_bloom.read().contains("safe-site.com"));

        state.whitelist_bloom.write().insert("trusted-cdn.com");
        assert!(state.whitelist_bloom.read().contains("trusted-cdn.com"));
        ...
        let parsed = crate::dns::parser::parse_dns_query(valid_query);
        ...
        {
            active_upstreams[1].errors.store(5, Ordering::Relaxed);
        ...
        state.log_config_decision(
            ...
            5,
            serde_json::json!({
                "audit": { "from": "PENDING", "to": "PASSED" },
                "subsystems": { "from": "154_TESTS", "to": "155_TESTS" }
            }),
        );
        ...
        state.metrics.requests.fetch_add(100, Ordering::Relaxed);
        state.metrics.cache_hits.fetch_add(85, Ordering::Relaxed);
        state.metrics.cache_misses.fetch_add(15, Ordering::Relaxed);
        state.metrics.threat_blocks.fetch_add(5, Ordering::Relaxed);
        assert_eq!(state.metrics.requests.load(Ordering::Relaxed), 100);
        assert_eq!(state.metrics.cache_hits.load(Ordering::Relaxed), 85);

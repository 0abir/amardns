...
impl DnsCache {
    ...
    pub async fn get_with_swr(
        ...
        if let Some(entry) = self.cache.get(&key).await {
            ...
            if elapsed_secs < entry.original_ttl {
                ...
                Self::decrement_response_ttl(&mut out, elapsed_secs);
    ...
    pub async fn insert(&self, qname: &str, qtype: u16, raw_response: Vec<u8>, ttl: u32) {
        self.insert_with_grace(qname, qtype, raw_response, ttl, 300)
            .await;
    }

    pub async fn insert_with_grace(
        &self,
        qname: &str,
        qtype: u16,
        raw_response: Vec<u8>,
        ...
        // RebindGuard: Never cache responses containing private/internal IPs unless exempt
        if !crate::dns::parser::is_rebind_exempt_domain(qname)
            && crate::dns::parser::extract_rebind_ip(&raw_response).is_some()
        {
            return;
        }
    ...
    /// Decrements record TTLs in the wire response by elapsed_secs according to RFC 1035 / RFC 2181.
    /// Ensures minimum remaining TTL of 1 second for active cache hits.
    pub fn decrement_response_ttl(wire: &mut [u8], elapsed_secs: u32) {
        if wire.len() < 12 || elapsed_secs == 0 {
            return;
        }
        ...
        for _ in 0..total_rrs {
            ...
            if rtype != 41 {
                ...
                let remaining_ttl = curr_ttl.saturating_sub(elapsed_secs).max(1);
    ...
    pub fn get_neg_stats(&self) -> (usize, u64) {
        (
            self.neg_cache.entry_count() as usize,
    ...
    pub async fn invalidate(&self, qname: &str, qtype: u16) {
        let key = Self::make_key(qname, qtype);
        self.cache.invalidate(&key).await;
    }
...
mod tests {
    ...
    async fn test_cache_swr_fresh_stale_and_miss() {
        ...
        cache
            .insert_with_grace("example.com", 1, dummy_resp.clone(), 10, 60)
            .await;

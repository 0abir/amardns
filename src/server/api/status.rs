...
pub async fn prometheus_metrics_handler(
    ...
    counter!(
        "amardns_threat_blocks",
        "Domains blocked by threat feed",
        threats
    );
    counter!(
        "amardns_dga_blocks",
        "Domains blocked by DGA heuristic",
        dga
    );
    counter!(
        "amardns_lookalike_blocks",
        "Domains blocked by lookalike detection",
        alike
    );
    counter!(
        "amardns_gsb_blocks",
        "Domains blocked by Google Safe Browsing",
        gsb
    );
    counter!(
        "amardns_rebind_blocks",
    ...
    counter!(
        ...
        "Plain DNS UDP/TCP port 53 queries received",
        m.plain_queries.load(Ordering::Relaxed)
    );
    counter!(
        "amardns_dnssec_validations",
        "DNSSEC cryptographic validations evaluated",
        m.dnssec_validations.load(Ordering::Relaxed)
    );
    counter!(
        "amardns_dnssec_secure",
        "DNSSEC cryptographically secure positive validations (AD=1)",
        m.dnssec_secure.load(Ordering::Relaxed)
    );
    counter!(
        "amardns_dnssec_bogus",
        "DNSSEC bogus or tampered signatures rejected",
        m.dnssec_bogus.load(Ordering::Relaxed)
    );
    counter!(
        "amardns_dnssec_insecure",
        "DNSSEC unsigned domain validations",
        m.dnssec_insecure.load(Ordering::Relaxed)
    );
    counter!(
        "amardns_dnssec_denial_of_existence",
        "DNSSEC NSEC/NSEC3 cryptographic denial-of-existence proofs verified",
        m.dnssec_denial_of_existence.load(Ordering::Relaxed)
    );
    counter!(
        "amardns_fast_neg_hits",
        "Fast-negative filter cache hits",
        fneg
    );
    counter!(
        "amardns_swr_serves",
    ...
    counter!(
        "amardns_prefetch_triggers",
        "Prefetch triggers fired",
        pfetch_t
    );
    counter!(
        "amardns_prefetch_hits",
        "Prefetch hits (resolved before query)",
        pfetch_h
    );
    counter!(
        "amardns_ttl_guard_blocks",
        "TTL manipulation guard blocks",
        ttlg
    );
    ...
    counter!(
        "amardns_canary_hits",
        "Canary domain detection hits",
        canary_h
    );
    counter!(
        "amardns_wal_dropped_entries",
        "WAL entries dropped under backpressure",
        dropped
    );
    counter!(
        "amardns_ai_training_cycles",
        "AI brain online training cycles",
        brain_cyc
    );
    counter!(
        "amardns_singleflight_coalesced",
        "Queries deduplicated by singleflight (reused in-flight result)",
        state.upstreams.coalesced_count()
    );
    counter!(
        "amardns_latency_micros",
    ...
    gauge!(
        "amardns_rps_peak",
        "Peak requests per second (lifetime)",
        rps_peak
    );
    ...
    gauge!(
        "amardns_cache_bytes",
        "Current DNS cache memory usage (bytes)",
        cache_bytes
    );
    gauge!(
        "amardns_threat_bloom_domains",
        "Threat bloom filter domain count",
        bloom_cnt
    );
    gauge!(
        "amardns_whitelist_bloom_domains",
        "Whitelist bloom filter domain count",
        wl_bloom
    );
    gauge!(
        "amardns_healthy_upstreams",
        "Number of healthy upstream resolvers",
        healthy_up
    );
    ...
    out.push_str(&format!(
        "amardns_latency_bucket{{le=\"5\"}} {}\n",
        lat_s1 + lat_1_5
    ));
    out.push_str(&format!(
        "amardns_latency_bucket{{le=\"15\"}} {}\n",
        lat_s1 + lat_1_5 + lat_5_15
    ));
    out.push_str(&format!(
        "amardns_latency_bucket{{le=\"50\"}} {}\n",
        lat_s1 + lat_1_5 + lat_5_15 + lat_1550
    ));
    out.push_str(&format!(
        "amardns_latency_bucket{{le=\"+Inf\"}} {}\n",
...
pub fn build_status_response(state: &AppState, auth: AuthRole) -> Response {
    ...
    let fly_region = std::env::var("FLY_REGION").unwrap_or_else(|_| "sin".to_string());
    let fly_app = "AmarDNS".to_string();

    let machine_id_val = serde_json::Value::String(fly_machine_id.clone());
    ...
    let effective_threat_total = if exp_threat_total > 0 {
        exp_threat_total
    } else {
        threat_bloom_cnt
    };
    let effective_white_total = if exp_white_total > 0 {
        exp_white_total
    } else {
        wl_bloom_cnt
    };
    ...
    let major = 1 + (brain_cycles / 10000);
    let minor = (brain_cycles / 100) % 100;
    let patch = brain_cycles % 100;
    let brain_version = format!("{}.{:02}.{:02}", major, minor, patch);
    ...
    let (lat_sub1, lat_1_5, lat_5_15, lat_15_50, lat_above50) =
        state.metrics.get_latency_distribution();
    ...
    let open_cb_count = upstreams
        .iter()
        .filter(|u| !u.get("healthy").and_then(|v| v.as_bool()).unwrap_or(true))
        .count();

    let heatmap_guard = state.heatmap.read();
    let mut hot_domains: Vec<serde_json::Value> = heatmap_guard
        .iter()
        .map(|(d, r)| {
            let max_idx = r
                .hourly
    ...
    let status = serde_json::json!({
        ...
        "dnssec": {
            "validationsTotal": state.metrics.dnssec_validations.load(Ordering::Relaxed),
            "secure": state.metrics.dnssec_secure.load(Ordering::Relaxed),
            "bogus": state.metrics.dnssec_bogus.load(Ordering::Relaxed),
            "insecure": state.metrics.dnssec_insecure.load(Ordering::Relaxed),
            "denialOfExistence": state.metrics.dnssec_denial_of_existence.load(Ordering::Relaxed),
            "rootTrustAnchors": crate::dns::dnssec::TrustAnchorStore::global().count(),
            "status": "ACTIVE_CRYPTOGRAPHIC_ENGINE",
            "rfcStandard": "RFC 4034 / RFC 4035 / RFC 5155 / RFC 9276"
        },
        "protocols": {
        ...
        "singleflight": {
            "coalesced": state.upstreams.coalesced_count(),
            "inFlight": state.upstreams.in_flight_count()
        },
        "upstreams": upstreams,

...
pub async fn handle_get_ai_brain(
    ...
    let (_sample_score, _sample_verdict) =
        state.brain.evaluate_internal("telemetry.probe.internal");
    ...
    Json(serde_json::json!({
        ...
        "version": format!("{}.{:02}.{:02}", 1 + (brain_cycles / 10000), (brain_cycles / 100) % 100, brain_cycles % 100),
...
pub async fn ai_export(State(state): State<Arc<AppState>>, headers: HeaderMap) -> Response {
    let auth = check_auth(&state, None, &headers, "/api/ai/export");
    if !auth.is_admin() {
        return (
            StatusCode::UNAUTHORIZED,
            Json(serde_json::json!({ "ok": false, "error": "Admin authorization required" })),
        )
            .into_response();
    }
...
pub async fn ai_import(
    ...
    match state.brain.import_json(&raw_data) {
        Ok(count) => {
            state.log_action(
                "ai_brain_imported",
                &format!("Restored {} domains into AI brain", count),
            );
...
pub async fn ai_prune(State(state): State<Arc<AppState>>, headers: HeaderMap) -> Response {
    ...
    state.log_action(
        "ai_brain_pruned",
        &format!("Pruned {} noise domains from brain", pruned),
    );

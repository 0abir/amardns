...
pub async fn handle_get_blocklist(
    state: &AppState,
    key: Option<&str>,
    headers: &HeaderMap,
) -> Response {
    ...
    let clean = match crate::security::sanitizer::sanitize_domain(&raw) {
        ...
        Err(err) => {
            return (
                ...
                Json(
                    serde_json::json!({ "ok": false, "error": format!("invalid domain: {}", err) }),
                ),
            )
                .into_response();
        }
...
pub async fn handle_clear_blocklist(
    state: &AppState,
    key: Option<&str>,
    headers: &HeaderMap,
) -> Response {
...
pub async fn handle_clear_whitelist(
    state: &AppState,
    key: Option<&str>,
    headers: &HeaderMap,
) -> Response {
...
pub async fn handle_get_common(
    state: &AppState,
    key: Option<&str>,
    headers: &HeaderMap,
) -> Response {
...
pub async fn handle_clear_common(
    state: &AppState,
    key: Option<&str>,
    headers: &HeaderMap,
) -> Response {
            ...
            state.custom_blocklist.write().insert(
                ...
                crate::state::BlockEntry {
                    ...
                    reason: payload
                        .reason
                        .clone()
                        .unwrap_or_else(|| "ai_auto_block".to_string()),
                    source: "ai".to_string(),
                    tag: "AI".to_string(),
                    auto: true,
                    created_at: now,
...
pub async fn get_heatmap_top(State(state): State<Arc<AppState>>, headers: HeaderMap) -> Response {
...
pub async fn get_heatmap_top_key(
    ...
    if !auth.is_view_or_admin() {
        return (
            StatusCode::UNAUTHORIZED,
            Json(serde_json::json!({ "ok": false, "error": "Unauthorized: Invalid key" })),
        )
            .into_response();
    }
    let guard = state.heatmap.read();
...
pub fn handle_heatmap_lookup(
    state: &AppState,
    domain_opt: Option<&str>,
...
pub async fn dga_test(
    ...
    let auth = check_auth(&state, None, &headers, "/api/dga-test");
    if !auth.is_view_or_admin() {
        return (
            StatusCode::UNAUTHORIZED,
            Json(serde_json::json!({ "ok": false, "error": "Unauthorized: Authentication required" })),
        ).into_response();
    }
    handle_dga_test(&state, payload).into_response()
}
...
pub async fn dga_test_key(
    ...
    let auth = check_auth(&state, Some(&key), &headers, "/api/dga-test");
    if !auth.is_view_or_admin() {
        return (
            StatusCode::UNAUTHORIZED,
            Json(serde_json::json!({ "ok": false, "error": "Unauthorized: Invalid key" })),
...
fn handle_dga_test(state: &AppState, payload: DomainReq) -> Json<serde_json::Value> {
    ...
    let utility_proof = if is_dga {
        format!(
            "Zero-Day DGA intercepted (Entropy {:.2}); prevented algorithmic botnet connection",
            entropy
        )
    } else if is_alike {
...
pub async fn get_schedule(State(state): State<Arc<AppState>>, headers: HeaderMap) -> Response {
    ...
    handle_get_schedule(&state)
}

fn handle_get_schedule(state: &AppState) -> Response {
...
pub async fn add_schedule(
    ...
    handle_add_schedule(&state, body)
}

fn handle_add_schedule(state: &AppState, body: AddScheduleReq) -> Response {
    let clean_domain = match crate::security::sanitizer::sanitize_domain(&body.domain) {
        Ok(d) => d,
        Err(err) => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"ok":false,"error":format!("Invalid domain: {}", err)})),
            )
                .into_response();
        }
    };
    if body.start_hour > 23 || body.end_hour > 23 {
        return (
            StatusCode::BAD_REQUEST,
            Json(serde_json::json!({"ok":false,"error":"Hours must be 0-23"})),
        )
            .into_response();
    }
    ...
    let id = state.schedule_store.add_rule(
        clean_domain.clone(),
        body.start_hour,
        body.end_hour,
        body.tz_offset,
...
pub async fn delete_schedule(
    ...
    handle_delete_schedule(&state, id)
}

pub async fn delete_schedule_key(
    State(state): State<Arc<AppState>>,
    Path((id, key)): Path<(u64, String)>,
    headers: HeaderMap,
) -> Response {
    let auth = check_auth(&state, Some(&key), &headers, "/api/schedule");
    if !auth.is_admin() {
        return (
            StatusCode::UNAUTHORIZED,
            Json(serde_json::json!({"ok":false,"error":"Admin key required"})),
...
fn handle_delete_schedule(state: &AppState, id: u64) -> Response {

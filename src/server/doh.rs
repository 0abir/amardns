...
pub async fn process_dns_wire_packet_full(
        ...
        .await
    {
        crate::dns::cache::CacheLookupResult::Fresh(cached_wire) => {
            // RebindGuard: Intercept cached responses containing private IPs
            if state.blocking_enabled.load(Ordering::Relaxed)
                && !crate::dns::parser::is_rebind_exempt_domain(&q.name)
            {
                if let Some(rebind_ip) = crate::dns::parser::extract_rebind_ip(&cached_wire) {
                    state.cache.invalidate(&q.name, q.qtype).await;
                    state.cache.insert_negative(&q.name, q.qtype, 120).await;
        ...
        crate::dns::cache::CacheLookupResult::Stale(cached_wire) => {
            // RebindGuard: Intercept stale cached responses containing private IPs
            if state.blocking_enabled.load(Ordering::Relaxed)
                && !crate::dns::parser::is_rebind_exempt_domain(&q.name)
            {
                if let Some(rebind_ip) = crate::dns::parser::extract_rebind_ip(&cached_wire) {
    ...
    let resolve_result = match state.upstreams.resolve(query_wire).await {
        Some(res) => Some(res),
        None => match state.upstreams.resolve_race(query_wire).await {
            Some(r) => {
    ...
    if let Some((mut upstream_resp, upstream_name)) = resolve_result {
        // Strict RFC 5452 Section 5.3 Response Validation:
        // Question name, qtype, and qclass must match the outstanding query
        if let Some(parsed_resp) = crate::dns::parser::parse_dns_query(&upstream_resp) {
            if let Some(ref resp_q) = parsed_resp.question {
                if resp_q.name != q.name || resp_q.qtype != q.qtype || resp_q.qclass != q.qclass {
                    ...
                    state.log_anomaly(
                        ...
                        &format!(
                            "Mismatched upstream response from {}: expected {}:{}, got {}:{}",
                            upstream_name, q.name, q.qtype, resp_q.name, resp_q.qtype
                        ),
                    );
        ...
        // Feature 4: Record passive DNS timeline
        {
            ...
            if !ips.is_empty() {
                if let Some(drift) = state.passive_dns.record(&q.name, &ips) {
                    ...
                    // Reputable, CDN, Anycast, and common whitelisted domains legitimately rotate IPs
                    // across round-robin edge nodes on every query. Only log an anomaly if the domain
                    // is NOT exempt/whitelisted to avoid flooding logs with benign CDN load-balancing.
                    if !state.is_exempt(&q.name) {
                        state.log_anomaly(
                            "passive_dns_drift",
                            &format!(
...
pub async fn doh_json_handler(
    ...
    if let Some(cached_wire) = state.cache.get(&clean_domain, qtype, 0x1234).await {
        if state.blocking_enabled.load(Ordering::Relaxed)
            && !crate::dns::parser::is_rebind_exempt_domain(&clean_domain)
        {
            if let Some(rebind_ip) = crate::dns::parser::extract_rebind_ip(&cached_wire) {
                state.cache.invalidate(&clean_domain, qtype).await;
                ...
                let resp = serde_json::json!({
                    ...
                    "Answer": [],
                    "Comment": "AmarDNS Block: REBIND (DNS Rebinding to Private IP Prohibited)"
                });
                return Json(resp).into_response();
            }
        }
    ...
    if let Some((resp_bytes, upstream_name)) = upstream_res {
        // DNS Rebinding Protection for DoH JSON API
        if state.blocking_enabled.load(Ordering::Relaxed)
            && !crate::dns::parser::is_rebind_exempt_domain(&clean_domain)
        {
            if let Some(rebind_ip) = crate::dns::parser::extract_rebind_ip(&resp_bytes) {
                state.metrics.rebind_blocks.fetch_add(1, Ordering::Relaxed);
                state.metrics.threat_blocks.fetch_add(1, Ordering::Relaxed);
                state.fingerprint.record_response(client_ip, 3);
...
mod tests {
    ...
    #[tokio::test]
    async fn test_aerocache_dns_rebinding_interception_and_rejection() {
        let config = crate::config::Config::from_env();
        let state = Arc::new(AppState::new(config));

        // 1. Synthesize a DNS response containing a private rebind IP (127.0.0.1)
        let mut rebind_resp = vec![
            0x12, 0x34, 0x81, 0x80, // Response, RD=1, RA=1, NOERROR
    ...
    #[tokio::test]
    async fn test_upstream_response_question_mismatch_rejected() {
        // Create an upstream response with question: "attacker.com"
        let mut mismatched_resp = vec![
            0x12, 0x34, 0x81, 0x80, // Response, NOERROR
            0x00, 0x01, 0x00, 0x01, // QDCOUNT=1, ANCOUNT=1
            0x00, 0x00, 0x00, 0x00,
        ];
